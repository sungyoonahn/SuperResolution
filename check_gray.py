...
x_test_list = sorted(glob.glob(os.path.join(base_path, 'x_test_gray', '*.npy')))
y_test_list = sorted(glob.glob(os.path.join(base_path, 'y_test_gray', '*.npy')))
...
# 모델이 예측한 이미지(output)
y_pred = model.predict(x1_test.reshape((1, 44, 44)))
...
y_pred = np.clip(y_pred.reshape((176, 176)), 0, 1)
...
ax[1].set_xlabel("{:.2f} db".format(psnr1))

ax[2].set_title('output')
ax[2].imshow(y_pred)
psnr2 = cv2.PSNR(y_pred,y1_test)
ax[2].set_xlabel("{:.2f} db".format(psnr2))
